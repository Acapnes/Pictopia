...
export class PicAPI {
  ...
  public static async getPicsByExplore(picPaginationDto: PaginationDto): Promise<PicDto[]> {
    if(window.localStorage.getItem('access_token')){
      return await axios.post("http://localhost:3000/pics/account/explore",{
        currentPage: picPaginationDto.currentPage,
        postPerPage: picPaginationDto.postPerPage
      }).then((resp) => resp.data);
    }
    return await axios.post("http://localhost:3000/pics/explore",{
      currentPage: picPaginationDto.currentPage,
      postPerPage: picPaginationDto.postPerPage
    }).then((resp) => resp.data);
  }
  ...
  public static async getPicsByCategory(picPaginationDto: PaginationDto): Promise<PicDto[]> {
    if(window.localStorage.getItem('access_token')){
      return await axios.post("http://localhost:3000/pics/account/category",{
  ...
  public static async getPicsAlias(_id: string, picPaginationDto: PaginationDto): Promise<PicDto[]> {
    return await axios.post(`http://localhost:3000/pics/alias/${_id}`, picPaginationDto)
      .then((resp) => resp.data);
  }
