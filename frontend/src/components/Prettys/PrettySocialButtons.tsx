  ...
  return (
    <div className="flex flex-row justify-between text-blue-400 pr-1.5 pb-1.5">
      <div className="flex flex-row space-x-3 items-center truncate">
        ...
        {showUrl && (
          <a href={`//${socialUrl}`} className="truncate">
            {socialUrl}
          </a>
        )}
      </div>
      {showUrl && (
        <button
          onClick={async () => {
            await ModerationAPI.deleteUserSocial(
              ...
              if (loginResp.success === true) {
                const updatedItems = profileSocials.filter(
                  (social: UserDto["userSocials"][0]) =>
                    social.index !== socialIndex
                );
                setProfileSocials(updatedItems);
              }
            });
          }}
          className="px-2.5 hover:bg-extra-light-soft-black transition duration-200 rounded-sm"
        >
          <PrettyTrashIcon size={20} fill={"rgb(244, 114, 182)"} />
        </button>
      )}
    </div>
  );
